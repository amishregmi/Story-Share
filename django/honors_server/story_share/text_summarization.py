def text_summarization_function(whole_string_content):
	...
	word_freq_dict = get_word_frequency_table(whole_string_content)

	sentence_scores = score_sentences(whole_string_content, word_freq_dict)

	threshold_val = get_average_score(sentence_scores)

	summary = gen_summary(sent_tokenize(whole_string_content), sentence_scores, 1.25*threshold_val)
